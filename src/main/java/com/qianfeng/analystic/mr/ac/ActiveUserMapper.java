package com.qianfeng.analystic.mr.ac;
 ...
 * @Description: 活跃的用户和活跃的总用户统计的mapper类, 需要launch事件中的uuid为一个数
 */
public class ActiveUserMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    private KpiDimension newActiveKpi = new KpiDimension(KpiType.ACTIVE_USER.kpiName);
    private KpiDimension browserNewActiveKpi =new KpiDimension(KpiType.BROWSER_ACTIVE_USER.kpiName);
    @Override
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        statsCommonDimension.setKpiDimension(newActiveKpi);
        ...
        for (PlatformDimension pl : platformDimensions) {
            ...
            for(BrowserDimension br: browserDimensionList){
                statsCommonDimension.setKpiDimension(browserNewActiveKpi);
                this.k.setStatsCommonDimension(statsCommonDimension);
